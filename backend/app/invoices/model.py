...
class Invoice(Base):
    ...
    branch_id = sql.Column(UUIDType(binary=False))
    user_id =  sql.Column(UUIDType(binary=False))
    payment_status = sql.Column('payment_status', sql.Enum(PaymentStatusEnum), default=PaymentStatusEnum.pending)
